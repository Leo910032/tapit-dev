...
}// app/dashboard/(dashboard pages)/contacts/page.jsx - FIXED for Firebase Auth
"use client"
import { useEffect, useState } from 'react';
import { useTranslation } from '@/lib/useTranslation';
import { useAuth } from '@/contexts/AuthContext'; // ✅ Use Firebase Auth
...
export default function ContactsPage() {
    ...
    useEffect(() => {
        const checkSharingPermissions = async () => {
            if (user?.uid) { // ✅ Use Firebase UID
                try {
                    const enabled = await checkTeamContactSharingEnabled(user.uid);
    ...
    // ✅ Updated saveScannedContact function for Firebase Auth
    const saveScannedContact = async (fields) => {
        try {
            ...
            const contactsRef = doc(collection(fireApp, "Contacts"), user.uid); // ✅ Use Firebase UID
    ...
    // ✅ Real-time Firebase listener - Updated for Firebase Auth
    useEffect(() => {
        const setupContactsListener = async () => {
            // Wait for auth to load
            if (authLoading) {
                console.log('⏳ Contacts: Auth still loading...');
                return;
            }

            // Check if user is authenticated
            if (!user?.uid) {
                console.log('❌ Contacts: No authenticated user');
                setLoading(false);
                return;
            }

            console.log('🔥 Setting up real-time contacts listener for user:', user.uid);
    ...
    }, [authLoading, user, loading, contacts.length, t]);
            ...
            <div className="flex items-center justify-center p-8 min-h-[400px]">
                <div className="flex flex-col items-center space-y-3">
                    ...
                    <span className="text-sm text-gray-600">
                        {authLoading ? 'Authenticating...' : 'Loading contacts...'}
                    </span>
                    {userData && (
                        <span className="text-xs text-green-600">
                            {t('contacts.welcome')} {userData.displayName || userData.username}!
                        </span>
                    )}
                </div>
            </div>
    ...
    return (
        <div className="flex-1 py-2 flex flex-col max-h-full overflow-y-auto pb-20">
            ...
            <div className="p-3 sm:p-4">
                {/* ✅ Mobile-Optimized Header with Firebase user data */}
                <div className="mb-4">
                    ...
                    <div className="flex items-center gap-3 mt-3 text-xs overflow-x-auto">
                        ...
                        {userData && (
                            <div className="flex items-center text-blue-600 whitespace-nowrap">
                                ...
                                <span className="hidden sm:inline">{t('contacts.firebase_auth')}</span>
                                <span className="sm:hidden">{t('contacts.auth')}</span>
                            </div>
                        )}
                        {teamSharingEnabled && (
                        ...
                        <div className="space-y-3">
                           ...
                           <div className="bg-blue-50 rounded-lg p-3 mb-4">
                                <div className="text-sm font-medium text-blue-900 mb-1">
                                    {userData?.displayName || userData?.username ? 
                                        `${userData.displayName || userData.username}, ${t('contacts.showing_contacts', { count: filteredContacts.length, total: contacts.length })}` :
            ...
            {showShareModal && (
                <ShareContactsModal
                    ...
                    userId={userData?.uid} // ✅ Use Firebase UID
                />
            )}
...
// ==========================================================
// Mobile-Optimized Contact Card Component (unchanged)
// ==========================================================
